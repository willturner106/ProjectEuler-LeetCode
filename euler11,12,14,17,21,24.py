def sqrt(num):
    initGuess = 2
    go = True
    while go:
        nextGuess = .5 * (initGuess + (num/initGuess))
        if abs(nextGuess - initGuess) < 0.001:
            go = False
        initGuess = nextGuess
    return round(nextGuess,3)

# ------------------------------------------------------ Problem 11 ------------------------------------------------------
grid = ["08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08".split(),
"49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00".split(), 
"81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65".split(), 
"52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91".split(), 
"22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80".split(), 
"24 47 32 60 99 03 45 02 44 75 33 53 78 36 84 20 35 17 12 50".split(),
"32 98 81 28 64 23 67 10 26 38 40 67 59 54 70 66 18 38 64 70".split(), 
"67 26 20 68 02 62 12 20 95 63 94 39 63 08 40 91 66 49 94 21".split(), 
"24 55 58 05 66 73 99 26 97 17 78 78 96 83 14 88 34 89 63 72".split(), 
"21 36 23 09 75 00 76 44 20 45 35 14 00 61 33 97 34 31 33 95".split(), 
"78 17 53 28 22 75 31 67 15 94 03 80 04 62 16 14 09 53 56 92".split(), 
"16 39 05 42 96 35 31 47 55 58 88 24 00 17 54 24 36 29 85 57".split(),
"86 56 00 48 35 71 89 07 05 44 44 37 44 60 21 58 51 54 17 58".split(), 
"19 80 81 68 05 94 47 69 28 73 92 13 86 52 17 77 04 89 55 40".split(), 
"04 52 08 83 97 35 99 16 07 97 57 32 16 26 26 79 33 27 98 66".split(), 
"88 36 68 87 57 62 20 72 03 46 33 67 46 55 12 32 63 93 53 69".split(), 
"04 42 16 73 38 25 39 11 24 94 72 18 08 46 29 32 40 62 76 36".split(),
"20 69 36 41 72 30 23 88 34 62 99 69 82 67 59 85 74 04 36 16".split(), 
"20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54".split(), 
"01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48".split()]

for i in range(20):
    grid[i] = [int(b) for b in grid[i]]
max = 0

#down
for a in range(17):
    for b in range(20):
        if grid[a][b] * grid[a+1][b] * grid[a+2][b] * grid[a+3][b] > max:
            max = grid[a][b] * grid[a+1][b] * grid[a+2][b] * grid[a+3][b]

#right
for a in range(20):
    for b in range(17):
        if grid[a][b] * grid[a][b+1] * grid[a][b+2] * grid[a][b+3] > max:
            max = grid[a][b] * grid[a][b+1] * grid[a][b+2] * grid[a][b+3]

#down and right
for a in range(17):
    for b in range(17):
        if grid[a][b] * grid[a+1][b+1] * grid[a+2][b+2] * grid[a+3][b+3] > max:
            max = grid[a][b] * grid[a+1][b+1] * grid[a+2][b+2] * grid[a+3][b+3]

for a in range(19,3,-1):
    for b in range(17):
        if grid[a][b] * grid[a-1][b+1] * grid[a-2][b+2] * grid[a-3][b+3] > max:
            max = grid[a][b] * grid[a-1][b+1] * grid[a-2][b+2] * grid[a-3][b+3]

print("11:",max)

# ------------------------------------------------------ Problem 12 ------------------------------------------------------

def numFactors(n):
    count = 0
    for i in range(1,int(sqrt(n))+1):
        if n % i == 0:
            count+=2
    return count

def anotherreallylongfunctionnamebecauseithinkitskindoffunny():
    i = 1
    tri = 0
    while True:
        tri = tri + i
        i = i+1
        if numFactors(tri) > 500:
            print("12:",tri)
            return
#anotherreallylongfunctionnamebecauseithinkitskindoffunny()

# ------------------------------------------------------ Problem 14 ------------------------------------------------------
"""
def lenCollatz(n):
    len = 1
    while n != 1:
        if n % 2 == 0:
            n = n/2
            len+=1
        else:
            n = n * 3 + 1
            len+=1
    return len

max = 0
maxC = 0
for i in range(1,1000000):
    if lenCollatz(i) > maxC:
        max = i
        maxC = lenCollatz(max)
print("14:",max)"""
# ------------------------------------------------------ Problem 17 ------------------------------------------------------

d = {0:"",1:"one",2:"two",3:"three",4:"four",5:"five",6:"six",7:"seven",8:"eight",9:"nine",10:"ten",11:"eleven",12:"twelve",13:"thirteen",14:"fourteen",15:"fifteen",16:"sixteen",17:"seventeen",
18:"eighteen",19:"nineteen",20:"twenty",30:"thirty",40:"forty",50:"fifty",60:"sixty",70:"seventy",80:"eight",90:"ninety",100:"hundred"}

total = 0

for i in range(1,21):
    print(d[i])
    total += (len(d[i]))

for i in range(21,100):
    print(d[int(str(i)[0]+"0")],d[int(str(i)[1])])
    total += len(d[int(str(i)[0]+"0")]) #tens place
    total += len(d[int(str(i)[1])]) # ones place

print(total)

for i in range(100,1000):
    t = d[int(str(i)[0])] + d[100]
    total += len(d[int(str(i)[0])]) + len(d[100]) #hundreds
    if str(i)[1] != "0" or str(i)[2] != "0":
        t += "and"
    if int(str(i)[1] + str(i)[2]) < 20:
        t += d[int(str(i)[1] + str(i)[2])]
    else:
        t += d[int(str(i)[1]+"0")]
        t += d[int(str(i)[2])]
    print(t)
    total += len(t)

total += len("onethousand")

print("17:", total)


# ------------------------------------------------------ Problem 21 ------------------------------------------------------
def sumDiv(n):
    l = []
    for i in range(1,n//2+1):
        if n % i == 0:
            l.append(i)
    return sum(l)

li = []
for i in range(1,10000):
    t = sumDiv(i)
    if sumDiv(t) == i and i != t:
        if not i in li:
            li.append(i)
            li.append(t)
print("21:",sum(li))

# ------------------------------------------------------ Problem 24 ------------------------------------------------------
"""
l = {}
for p in range(0,10):
    for q in range(0,10):
        for w in range(0,10):
            for e in range(0,10):
                for r in range(0,10):
                    for t in range(0,10):
                        for y in range(0,10):
                            for u in range(0,10):
                                for i in range(0,10):
                                    for o in range(0,10):
                                        a = {p,q,w,e,r,t,y,u,i,o}
                                        print([p,q,w,e,r,t,y,u,i,o])
                                        if (len(a) == 10):
                                            l.append("".join(a))
                                            print(l)
print("24:",sorted[l][999999])"""

# ------------------------------------------------------ Problem 30 ------------------------------------------------------

total = 0
for i in range(2,1000000):
    s = sum([int(l)**5 for l in str(i)])
    if s == i:
        total += s
print("30:",total)
